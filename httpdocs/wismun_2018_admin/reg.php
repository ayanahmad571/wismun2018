...
$boxsql = "SELECT *,(SELECT COUNT(DISTINCT(i_id)) FROM `indv_reg_dele`) as indv_del_app,
                        ...
                        <div class="panel panel-color panel-inverse">
                            ...
                            <div class="panel-body"> 
								...
								if(strtolower($boxrw['form_name'])== 'delegate'){
									echo $boxrw['del_app']." Application(s) "	;
								}
								if(strtolower($boxrw['form_name']) == 'individual'){
									echo $boxrw['indv_del_app']." Application(s) ";
								}
								echo '</p> <p>
...
if ($frres->num_rows > 0) {
    ...
    while($frrw = $frres->fetch_assoc()) {
		...
		echo '<button class="btn btn-info" data-toggle="modal" data-target="#A'.$frrw['reg_sch_hash_rel'].'">'.$frrw['reg_sch_name'].'</button><br><br>';
	}
								...
								if(strtolower($boxrw['form_name']) == 'individual'){
								$jpeg = "
							SELECT * FROM `indv_reg_dele` where i_valid =1";
							$frres = $conn->query($jpeg);
...
if ($frres->num_rows > 0) {
    ...
    while($frrw = $frres->fetch_assoc()) {
		#firts loop begins
		echo '<button class="btn btn-info" data-toggle="modal" data-target="#B'.$frrw['i_id'].'">'.$frrw['i_d_name'].' from '.$frrw['i_s_name'].'</button><br><br>';
	}
